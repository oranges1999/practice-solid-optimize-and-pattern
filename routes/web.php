...
Route::middleware('auth')->group(
    function () {
        ...
        Route::get('users/download-sample', [UserController::class, 'downloadSampleFile'])
            ->name('users.download-sample');
        Route::get('users/{user}', [UserController::class, 'specificEdit'])->name('users.show');
    }
);

Route::prefix('api')->as('api.')->middleware('auth')->group(
    function () {
        ...
        Route::get('/users/{user}', [ApiUserController::class, 'getUserData'])
            ->name('users.show');
        Route::put('/users/{user}/update', [ApiUserController::class, 'updateSpecificUser'])
            ->name('users.update-specific');
        Route::delete('/users/{user}/delete', [ApiUserController::class, 'deleteSpecificUser'])
            ->name('users.delete-specific');
    }
);
